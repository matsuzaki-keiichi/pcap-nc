...
$PCAPNC $OPTCLNT $OPTSEND $CHAN  $OPTNOSPW< test-spp.pcap --link-type=spw >outdir/test-rpl-out-nospw.pcap &

echo starting server
FIFO=/tmp/pcap-fifo
mkfifo $FIFO
$NC $OPTSERV <$FIFO | ../bin/pcap-rmap-target $CHAN $OPTNOSPW >$FIFO
rm $FIFO
