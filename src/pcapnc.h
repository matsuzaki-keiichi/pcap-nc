...
class pcapnc {
  ...
  int write_packet_record(uint32_t coarse_time, uint32_t nanosec, uint8_t outot_buf[], const uint8_t outbuf[], size_t outlen, const char *prog_name, const char *source_name);
  inline int write_packet_record(uint32_t coarse_time, uint32_t nanosec, const uint8_t outbuf[], size_t outlen, const char *prog_name, const char *source_name){
    return this->write_packet_record(coarse_time, nanosec, NULL, outbuf, outlen, prog_name, source_name);
  }
