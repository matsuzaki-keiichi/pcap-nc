...
../bin/pcap-replay $SVOPT2 $CHAN --receive-reply $FIFO2 < test-spp.pcap | $PCAPNC --no-stdin $SVOPT1 >$FIFO2 &
sleep 1
echo starting client
$PCAPNC $CLOPT --link-type=spw <$FIFO1 | ../bin/pcap-rmap-target $CHAN | ../bin/pcap-replay --interval=0.0 --original-time >$FIFO1
