...
$PCAPNC $OPTSERV $OPTSEND $CHAN $OPTNOSPW < test-spp.pcap --after=1.0 --check-reply --store-data=outdir/test-spp-out-nospw.pcap &

sleep 1
echo starting client
FIFO=/tmp/pcap-fifo
mkfifo $FIFO
$NC $OPTCLNT <$FIFO | ../bin/pcap-rmap-target $OPTNOSPW $CHAN --send-data=test-spp.pcap >$FIFO
rm $FIFO
